...
type ExecutionResult = { ok: true; measurement: ExecutionMeasurement } | { ok: false; reason: FailureReason; detail: string };
...
export function createFillEngine({ chain, publicClient, walletClient, testClient, signer }: FillEngineParams): FillEngine {
    ...
    const gasByRoute = new Map<string, Promise<bigint | null>>();
    ...
    async function run(request: ExecutionRequest): Promise<ExecutionResult> {
        ...
        try {
            ...
            if (request.legs.length > 0) {
                ...
                const swapped = await send(data.to, data.data);

                if (!swapped.ok) {
                    return swapped;
                }

                gasUsed += swapped.gasUsed;
                lastTxHash = swapped.txHash;
    ...
    function remembered(key: string, measure: () => Promise<bigint | null>): Promise<bigint | null> {
        let gas = gasByRoute.get(key);

        if (gas === undefined) {
            gas = exclusive(measure);
            gasByRoute.set(key, gas);
        }
