...
const orderJobSchema = z.object({ orderId: z.string().min(1), stage: z.enum(["deposit", "fill", "settle", "refund", "rebalance"]) });
...
// what execute can leave an order in that means "the deposit is held and goes back"
const REFUND_STAGE = new Set<OrderState>([OrderState.REFUNDING, OrderState.FILL_FAILED, OrderState.INSUFFICIENT_INVENTORY]);
    ...
    const worker = new Worker<OrderJob>(
        ...
        async (job, token) => {
            ...
            if (stage === "deposit") {
                ...
                try {
                    ({ status, order } = await stages.awaitDeposit(orderId));
                } catch (error) {
                ...
                // a deposit that arrived too late is not filled, but it is still the user's
                if (status === "expired" && order.deposit !== null) {
                    stage = "refund";
                ...
                await job.updateData({ orderId, stage });
            }
            ...
            if (stage === "fill") {
                ...
                // the swap is done and the user is owed the payout: come back for that alone
                if (order.state === OrderState.PAYOUT_FAILED) {
                    return wait("fill");
                }

                if (REFUND_STAGE.has(order.state)) {
                    await job.updateData({ orderId, stage: "refund" });
                    stage = "refund";
                }
            ...
            if (stage === "refund") {
                const order = await stages.refund(orderId);

                return order.state === OrderState.REFUND_FAILED ? wait("refund") : undefined;
            }
