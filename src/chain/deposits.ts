...
export type Deposit = {
    ...
    token: Address;
    amount: bigint;
    // who sent it: the only address a refund can go back to
    from: Address;
    txHash: Hash;
    logIndex: number;
    blockNumber: bigint;
...
type TransferLog = { args: { from?: Address; value?: bigint }; blockNumber: bigint | null; logIndex: number | null; transactionHash: Hash };
...
export async function findDeposit({ client, chainId, token, to, amount, fromBlock, claimed }: FindDepositParams): Promise<Deposit | null> {
    ...
    for (const log of logs) {
        if (log.args.value !== amount || log.args.from === undefined || log.blockNumber === null || log.logIndex === null) {
            continue;
        }
...
// a transfer the solver may or may not have sent: found by who, to whom, how much, and from
// which block. this is how a process that died mid-send learns what the last one did.
export async function findTransfer(params: {
    ...
    amount: bigint;
    fromBlock: bigint;
}): Promise<{ txHash: Hash; blockNumber: bigint } | null> {
    const { client, token, from, to, amount, fromBlock } = params;
    const head = await client.getBlockNumber({ cacheTime: 0 });
    const logs = await getLogsInRanges<TransferLog>(client, { address: token, event: TRANSFER, args: { from, to }, fromBlock, toBlock: head });
    const found = logs.find((log) => log.args.value === amount && log.blockNumber !== null);

    return found === undefined ? null : { txHash: found.transactionHash, blockNumber: found.blockNumber! };
}
