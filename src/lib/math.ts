...
// floor of the square root, by newton's method on integers. exact, and the only way to scale a
// buffer with the square root of a wait without leaving bigint.
export function isqrt(value: bigint): bigint {
    if (value < 0n) {
        throw new Error("isqrt of a negative");
    }
